...
for i in range(max_length):
    pes_vanilla.append(positional_embedding(i, MODEL_SIZE))

pes_vanilla = np.concatenate(pes_vanilla, axis=0)
pes_vanilla = tf.constant(pes_vanilla, dtype=tf.float32)
pes = pes_vanilla
...
class TaskSolver(tf.keras.Model):
    ...
    def call(self, n_input):
        ...
        n_output = n_output+pes[:n_output.shape[1], :]
...
while True:
    ...
    for _ in range(BATCH_SIZE):
        ...
        num_nums = tf.random.uniform(shape=[],minval=1,maxval=(VEC_AMOUNT-1)//2+1,dtype=tf.int64)
        allnums = tf.random.shuffle(tf.range(0,VEC_AMOUNT,dtype=tf.int64))
        chosen_nums = allnums[:num_nums]
        
        num_hots = tf.zeros([VEC_AMOUNT],dtype=tf.int64)
        num_hots = tf.tensor_scatter_nd_update(num_hots,tf.expand_dims(chosen_nums,axis=-1),tf.ones_like(chosen_nums))
        
        color_subst = tf.random.shuffle(tf.range(0,N_COLORS,dtype=tf.int32))        
